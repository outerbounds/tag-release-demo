...
from metaflow import FlowSpec, step, IncludeFile, conda_base, Flow, current

@conda_base(python='3.8.10',
            libraries={'scikit-learn': '1.0.2',
                       'pandas': '1.2.5',
                       'graphviz': '2.48.0'})
class TreeCandidates(FlowSpec):
    ...
    def eval(self):
        ...
        self.accuracy = float(accuracy_score(self.test_target, self.pred))
        print('Accuracy', self.accuracy)

        if self.accuracy > 0.8:
            print('Tagging run as a promising candidate')
            run = Flow(current.flow_name)[current.run_id]
            run.add_tag('promising_tree')
